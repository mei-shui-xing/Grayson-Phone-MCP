...
The MCP server exposes 60 tools across 15 categories. For full JSON-RPC schemas, detailed usage examples, and implementation notes, see [MCP_TOOLS.md](MCP_TOOLS.md).
...
| Tool | Description | Required Params | Optional Params |
|------|-------------|-----------------|-----------------|
| `android_type_append_text` | Append via InputConnection; verified ACTION_SET_TEXT fallback when unavailable | `node_id` (string), `text` (string) | `typing_speed` (int, ms, default 250), `typing_speed_variance` (int, ms, default 50) |
| `android_type_insert_text` | Insert via InputConnection; verified ACTION_SET_TEXT fallback when unavailable | `node_id` (string), `text` (string), `offset` (int) | `typing_speed` (int, ms, default 250), `typing_speed_variance` (int, ms, default 50) |
| `android_type_replace_text` | Replace via InputConnection; verified ACTION_SET_TEXT fallback when unavailable | `node_id` (string), `search` (string), `new_text` (string) | `typing_speed` (int, ms, default 250), `typing_speed_variance` (int, ms, default 50) |
| `android_type_clear_text` | Clear via InputConnection; verified ACTION_SET_TEXT fallback when unavailable | `node_id` (string) | — |
...
- **[MCP_TOOLS.md](MCP_TOOLS.md)** — Full MCP tools documentation with JSON-RPC schemas, usage examples, error codes, and implementation notes for all 60 tools

...
### USB and AI-assisted setup

For first-time installation and troubleshooting, connect the phone to a computer over USB, enable USB debugging, and use a trusted AI coding assistant with the repository's PowerShell scripts. An assistant can help verify ADB authorization, build and install the APK, inspect non-sensitive permission state, create temporary port forwarding, collect redacted logs, and run harmless smoke tests.

This project is an Alpha and does not provide device-by-device support for OEM Android variants, VPN/proxy configurations, drivers, Android SDK installations, or individual computer environments. Provide errors, redacted logs, and these docs to an AI assistant for self-service diagnosis. This does not authorize an AI to bypass Android security boundaries: the device owner must personally handle lock-screen credentials, biometrics, account sign-in, OTPs, payments, permission dialogs, and every other sensitive confirmation. Never post bearer tokens, credentials, or private conversations in an issue.
